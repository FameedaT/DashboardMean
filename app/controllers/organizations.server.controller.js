...
exports.list = function (req, res) {
    Organization.find().sort('-created').populate('user', 'displayName').populate('projects').populate('owner', 'firstName lastName').exec(function (err, organizations) {
        if (err) {
            return res.status(400).send({
                message: errorHandler.getErrorMessage(err)
            });
...
exports.organizationByID = function (req, res, next, id) {
    Organization.findById(id).populate('user', 'displayName').populate('members', 'firstName lastName').populate('projects').populate('owner', 'firstName lastName').exec(function (err, organization) {
